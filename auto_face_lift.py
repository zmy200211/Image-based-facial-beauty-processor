...
def IDW_424(input_points, output_points, map_x, map_y):
    u = 1
    # 遍历人脸区域中的每一个像素位置
    for j in range(input_points[0][1], input_points[2][1]):
        for i in range(input_points[0][0], input_points[2][0]):
            x = 0
            y = 0
            sum_w = 0
            # 遍历输入控制点像素位置
            ...
            y = y / sum_w
            # 将x, y控制在图像大小范围内
            x = clip(x, 0, input_points[2][0] - 1)
            y = clip(y, 0, input_points[2][1] - 1)
            map_x[j, i] = x
            map_y[j, i] = y
    return map_x, map_y
...
def auto_face_lift_424(scr_image, image, faces, all_face_points, intensity):
    ...
    for people in range(len(faces)):
        ...
        output_points = [(min_x, min_y), (min_x, max_y), (max_x, max_y), (max_x, min_y),
                         p5, p61, p7, p81, p9]
        # 有多张人脸的情况
        map_x, map_y = IDW_424(input_points, output_points, map_x, map_y)
    # 通道拆分
